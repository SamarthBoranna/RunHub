    ...
    try:
        ...
        if not user:
            user = User(
                ...
                # api_key will be auto-generated by default
            )
            db.session.add(user)
        else:
            ...
            # Regenerate API key if it doesn't exist
            if not user.api_key:
        ...
        return redirect(f"{FRONTEND_URL}?user_id={user.id}&api_key={user.api_key}")
...
def authenticate_request(user_id):
    """Verify the API key belongs to the user ID in the path"""
    api_key = request.headers.get('X-API-Key')
    if not api_key:
        return None
    
    # Check if API key belongs to this user
